...
// --- Componentes Internos (No exportados para mantener limpia la API) ---
...
const ShaderMaterial = ({ source, uniforms, maxFps = 60 }: { source: string; uniforms: Uniforms; maxFps?: number }) => {
    ...
    const preparedUniforms = useMemo(() => {
        ...
        for (const key in uniforms) {
            ...
            if (type === "uniform1f") res[key] = { value };
            if (type === "uniform1fv") res[key] = { value };
            if (type === "uniform3fv") res[key] = { value: value.map((v: number[]) => new THREE.Vector3().fromArray(v)) };
            if (type === "uniform2f") res[key] = { value: new THREE.Vector2().fromArray(value) };
        }
        return res;
    }, [uniforms, size]);
...
const Shader: React.FC<{ source: string; uniforms: Uniforms; maxFps?: number }> = ({ source, uniforms, maxFps }) => (
